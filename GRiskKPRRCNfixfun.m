function [ktildez0,cz0,vz0,kmax,kbar,ksd,kssM,kssno,cbar,csd,ibar,isd,ybar,ysd,v1,v2,v3,v4,iter,trashit,trashmax,v,ktildeposition,ktilde,n,c,z,kmin,sigma,sdz,psi] = GRiskKPRRCNfixfun(sdz,sigma,theta,psi,A,alpha,rho,beta,mu,delta,gp,gpz,T,Ti,itermax,k,v,UsekssTh,kssTh,nssTh,Reps)
...
for i=1:gp
    for j=1:gp
        for m=1:gpz
            ...
            cc = (1+z(m))*A*(k(i))^alpha*n^(1-alpha)+(1-delta)*k(i)+mu-k(j);
            u0 = (cc^(1-sigma)*(1-psi*(1-sigma)/(1+1/theta)*n^(1+1/theta))^sigma-1)/(1-sigma);

            if 1-psi*(1-sigma)/(1+1/theta)*n^(1+1/theta) > 0 && cc > 0 && isreal(u0)
                U0(i,j,m)=u0;
            else
                U0(i,j,m)=-inf;
            end
...
for i=1:gp
    for m=1:gpz
        c(i,m)=(1+z(m))*A*(k(i))^alpha*n^(1-alpha)+(1-delta)*k(i)+mu-ktilde(i,m);
    end
end
...
for r = 1:Reps
    ...
    Ysim=A*(1+Zsim).*(Ksim.^alpha).*(n^(1-alpha));
