...
sdzvect = [.1178; .1013; .0853; .0610; .0486; .0154];
...
gp=4000 % 4000 even number of gridpoints k-vector
...
for i=1:length(sigmavect)
    psivect(i)=cssTh^(-sigmavect(i))*(1-alpha)*A*kssTh^alpha/(nssTh^(alpha+theta));
end
...
tv = datetime('now', 'Format','yyyy_MM_dd_HH_mm');
filename = sprintf('GRiskADSe4Nfix_%s.mat',tv);
