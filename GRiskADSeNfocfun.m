function [ktildez0,noptz0,cz0,vz0,kmax,kbar,ksd,kssM,kssno,nbar,nsd,cbar,csd,ibar,isd,ybar,ysd,v1,v2,v3,v4,iter,trashit,trashmax,v,ktildeposition,ktilde,n,c,z,kmin,sigma,sdz,psi] = GRiskADSeNfocfun(sdz,sigma,theta,psi,A,alpha,rho,beta,mu,delta,gp,gpz,T,Ti,itermax,k,v,UsekssTh,kssTh,nssTh,Reps)
% Solves the Markow growth problem with ADS-utility by iterating on a
% discrete grid.
% Resource constraint
% Uncertain productivity exp(z0, gpz different states
...
for i=1:gp
    for j=1:gp
        for m=1:gpz
            ...
            if sigma==0
                nopt = ((1-alpha)*A*(exp(z(m)))*(k(i))^alpha/psi)^(1/(alpha+theta));
                noptcondj(i,j,m)=nopt;
            else
                solnsqrt = fzero( @(nvar) (A*(exp(z(m)))*(k(i))^alpha*(real(nvar^2))^(1-alpha)+(1-delta)*k(i)-k(j)-(((1-alpha)*A*(exp(z(m)))*(k(i))^alpha*(real(nvar^2))^(-alpha-theta))/psi)^(1/sigma)), sqrt(nssTh)*0+.01, options);
                nopt = solnsqrt^2; % Square in function to only get positive solutions
                noptcondj(i,j,m)=nopt;
            end


            if nopt >= 0 && (exp(z(m)))*A*(k(i))^alpha*nopt^(1-alpha)+(1-delta)*k(i)+mu-k(j) >= 0
                U0(i,j,m)=((exp(z(m)))*A*(k(i))^alpha*nopt^(1-alpha)+(1-delta)*k(i)+mu-k(j))^(1-sigma)/(1-sigma)-psi*nopt^(1+theta)/(1+theta);
            else
                U0(i,j,m)=-inf;
            end
            ...
            %     U0(i,j,m)=-inf;
            % elseif (exp(z(m)))*A*(k(i))^alpha*nopt^(1-alpha)+(1-delta)*k(i)+mu-k(j) < 0 % Non-pos consumption
            %     U0(i,j,m)=-inf;
            % else
            %     U0(i,j,m)=((exp(z(m)))*A*(k(i))^alpha*nopt^(1-alpha)+(1-delta)*k(i)+mu-k(j))^(1-sigma)/(1-sigma)-psi*nopt^(1+theta)/(1+theta);
            % end
...
for i=1:gp
    for m=1:gpz
        ...
        c(i,m)=(exp(z(m)))*A*(k(i))^alpha*(n(i,m))^(1-alpha)+(1-delta)*k(i)+mu-ktilde(i,m);
    end
end
clear noptcondj;
