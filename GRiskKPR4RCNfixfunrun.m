...
gpz=41; % 51 19 odd number>=5 Stops running properly if too large, and optimal k-rule goes out of bounds with so high shock values
...
GBRAMreq=2*2*(gp^2*gpz)*8/1024^3 % 8 bit with double precision
...
clear S_KTILDEZ0n S_CZ0n S_VZ0n S_KMAXn S_KBARn S_KSDn S_KSSMn S_KSSNOn S_CBARn S_CSDn S_IBARn S_ISDn S_YBARn S_YSDn S_V1n S_V2n S_V3n S_V4n S_ITERn S_TRASHITn S_TRASHMAXn Vn Ktildepositionn Ktilden Nn Cn Zn S_KMINn S_SDZn S_SIGMAn S_PSIn;

tv = datetime('now', 'Format','yyyy_MM_dd_HH_mm');
filename = sprintf('GRiskKPR4RCNfix_%s.mat',tv);
