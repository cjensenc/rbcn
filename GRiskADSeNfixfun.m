function [ktildez0,cz0,vz0,kmax,kbar,ksd,kssM,kssno,cbar,csd,ibar,isd,ybar,ysd,v1,v2,v3,v4,iter,trashit,trashmax,v,ktildeposition,ktilde,n,c,z,kmin,sigma,sdz,psi] = GRiskADSeNfixfun(sdz,sigma,theta,psi,A,alpha,rho,beta,mu,delta,gp,gpz,T,Ti,itermax,k,v,UsekssTh,kssTh,nssTh,Reps)
% Solves the Markow growth problem with ADS-utility by iterating on a
% discrete grid.
% Resource constraint
% Uncertain productivity z, n=gpz different states
% Constant work effort
...
for i=1:gp
    for j=1:gp
        for m=1:gpz
            ...
            cc = (exp(z(m)))*A*(k(i))^alpha*n^(1-alpha)+(1-delta)*k(i)+mu-k(j);

            if  cc >= 0
                U0(i,j,m)=(cc)^(1-sigma)/(1-sigma)-psi*n^(1+theta)/(1+theta);
            else
                U0(i,j,m)=-inf;
            end
...
for i=1:gp
    for m=1:gpz
        c(i,m)=(exp(z(m)))*A*(k(i))^alpha*n^(1-alpha)+(1-delta)*k(i)+mu-ktilde(i,m);
    end
end
...
for r = 1:Reps
    ...
    Ysim=A*(exp(Zsim)).*(Ksim.^alpha).*(n^(1-alpha));
